      ...
      if(filters.length != 0){
        for(var t = 0; t <= filters.length -1; t++){
          if(filters[t] == items[x]["sex"] && filterIds.indexOf(items[x]["id"]) == -1){
            ...
            productsList.innerHTML += "<a href='items.php?id="+items[x]["id"]+"' class='col-sm-4 items' style='background-image: url("+items[x]["productImg"]+")'><div class='merken-productnaam'>"+items[x]["merken"]+" - "+items[x]["productnaam"]+" €"+items[x]["prijs"]+",00</div></a>";          }
          if(filters[t] == items[x]["kleding"] && filterIds.indexOf(items[x]["id"]) == -1){
            filterIds.push(items[x]["id"]);
            productsList.innerHTML += "<a href='items.php?id="+items[x]["id"]+"' class='col-sm-4 items' style='background-image: url("+items[x]["productImg"]+")'><div class='merken-productnaam'>"+items[x]["merken"]+" - "+items[x]["productnaam"]+" €"+items[x]["prijs"]+",00</div></a>";          }
          if(filters[t] == items[x]["merken"] && filterIds.indexOf(items[x]["id"]) == -1){
            filterIds.push(items[x]["id"]);
            productsList.innerHTML += "<a href='items.php?id="+items[x]["id"]+"' class='col-sm-4 items' style='background-image: url("+items[x]["productImg"]+")'><div class='merken-productnaam'>"+items[x]["merken"]+" - "+items[x]["productnaam"]+" €"+items[x]["prijs"]+",00</div></a>";          }
        }
      }else{
